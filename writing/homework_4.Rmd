...
```{r unsolved_df}
unsolved <- homicides %>%  group_by(city_name) %>% 
  summarise(total_homicides = sum(!is.na(homicides$total_homicides)),
            unsolved_homicides = sum((homicides$unsolved_homicides == "TRUE")))
...
unnest(all_homicides, .drop = TRUE)
```
...
prop_df %>% 
  ...
  geom_errorbar(aes(ymin = (estimate - conf.low), 
                    ymax = (estimate + conf.high)), width = 0) + 
  coord_flip() + 
